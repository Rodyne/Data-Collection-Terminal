...
enum MIFARE_Misc	// MIFARE constants that does not fit anywhere else
{
	MF_ACK			= 0xA,	// The MIFARE Classic uses a 4 bit ACK/NAK. Any other value than 0xA is NAK.
	MF_KEY_SIZE	= 6			// A Mifare Crypto1 key is 6 uint8s.
};
...
typedef struct		// A struct used for passing a MIFARE Crypto1 key
{
	uint8	keybyte[MF_KEY_SIZE];
} tkey;

uint8 RFIDBuf[64];

// Variables used in this module also available to other modules where extern delared in header
tkey  key;
tuid  uid;
char  RFIDData[32];
...
uint8 PCD_Authenticate(uint8 authCommand, uint8 blockAddr)
{
	...
	for (i = 0; i < MF_KEY_SIZE; i++) 	// 6 key uint8s
	{
...
uint8 PICC_ReadBlock(uint8 blockAddr)
{
  // Init MFRC522 card
	...
	PCD_SetRegisterBitMask(TxControlReg, 0x03); // turn antenna on
	// reader now ready

  // Transmits REQA or WUPA commands to wake up and init tag. When two PICCs are in the field may get STATUS_TIMEOUT
	uint8 bufferATQA[2];
	...
	if ( PICC_Select() != STATUS_OK) return 0; // fail if could not select tag!!

	// SerialNo and card type is now available in struct uid

	if (uid.sak != 0x08)	return 0; // incompatable card types, I'm only supportting the common MIFARE_1K (S50)

	if(blockAddr==0) return 1; // just return the uid
	
	uint8 trailerBlock = 7; // reading sector #1, covering block #4 up to and including block #7
	buffSize = 18;					// 16 bytes + 2 CRC

	if ( PCD_Authenticate(PICC_CMD_MF_AUTH_KEY_A, trailerBlock) != STATUS_OK) return 0; // Authenticate using key A, return if Auth FAILS!

/*
 * Reads 16 uint8s (+ 2 uint8s CRC_A) from the active PICC.
 *
 * For MIFARE Classic the sector containing the block must be authenticated before calling this function.
 *
 * The MF0ICU1 returns a NAK for higher addresses.
 * The MF0ICU1 responds to the READ command by sending 16 uint8s starting from the page address defined by the command argument.
 * For example; if blockAddr is 03h then pages 03h, 04h, 05h, 06h are returned.
 * A roll-back is implemented: If blockAddr is 0Eh, then the contents of pages 0Eh, 0Fh, 00h and 01h are returned.
 * The buffer must be at least 18 uint8s because a CRC_A is also returned.
 ...
 */
	...
	if (PCD_TransceiveData(RFIDBuf, 4, RFIDBuf, &buffSize, NULL, 0, 1) != STATUS_OK ) return 0; // Transmit buffer and validate CRC_A on response
	...
	// PICC_HLT not required as the reader will switch off the RF and card will not have power
