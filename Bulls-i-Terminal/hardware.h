...
#define NVM_END		(void*) (0xBD000000 + BMXPFMSZ) // End Address of PIC321xx FLASH Memory. (using flash to emulate EEPROM)
...
struct config_fields// readable structure for our config variables (read from RFID sector 1)
{
  ...
  char    UserName[16];     // readable name of who is logged in
  uint32  PowerOffTimeout;  // how many seconds of innactivity shut down the device
};

typedef union // config variables are also read/written to non-volatile flash which is a 32 bit word so use a union to map one to the other
{
  struct config_fields field; // define cfg as 4 x char[16] fields plus some bytes suitable for actually using
  uint32 NVM[17];							// also define cfg as 17 x longints suitable for reading/writing to non-volatile memory
} Tconfiguration;

extern Tconfiguration	config; // declare system global vars, not really supposed to do in header but seems more logical here
