 ...
 * ie the CPU (PIC32MX250F128D), HW initialisation, KEYBOARD, BEEPER + Low level drivers for Power saving, SPI and SERIAL
*/

#define _SUPPRESS_PLIB_WARNING	// Get rid of annoying XC32 plib depreciation warnings
#define _DISABLE_OPENADC10_CONFIGPORT_WARNING // yet another annoying warning
...
#pragma config		FVBUSONIO = OFF    // VBUS input from USB
...
void InitPIC32(void) // Initialise PIC32 and terminal io pins
{
  ...
  // do before we set SPI (as enabling USB uses an SPI pin and display wont work - need to investigate)
  ...
  // Configure UART2 (higher priority than keyboard)
  ...
  UARTEnable(WIFI_UART, UART_ENABLE_FLAGS(UART_PERIPHERAL | UART_RX | UART_TX) );
	// configure UART2 Interrupt (initialy off)
  ...
  INTConfigureSystem(INT_SYSTEM_CONFIG_MULT_VECTOR); // Configure for Multi-Vectored Interrupts
	...
	PeripheralReset();	// Issue hard reset to LCD and RFID to initialise

	INTEnableInterrupts(); // globally enable interrupts but all are currently configured off until required
}
...
int8 WaitEvent(uint16 TimeoutSeconds)
{
  const  uint8 ScanCode[16] = { K_1, K_2, K_3, K_OK, K_4, K_5, K_6, K_CANCEL, K_7, K_8, K_9, K_UP, K_DECIMAL, K_0, K_DEL, K_DN }; // map scan 2 code
	...
	uint32 PowerTimeout = 1200;   // 20 minutes
	Backlight(ON);
