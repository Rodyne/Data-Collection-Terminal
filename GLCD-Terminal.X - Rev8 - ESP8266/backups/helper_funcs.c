...
{
	uint32  timeout = prg.InputTimeoutMs; // set timeout
	uint8   dx,key;
	uint8   len=0;
	...
	while(1)
	{
		if(timeout==0) return 0; // timed out
		if(KeyPressed())
		{
			key=ReadKey();
			if (key>='0' && key<='9')
...
{
	uint16 timeleft = WIFI_TIMEOUT;

	if(!WIFI_LINK()) // no connection
		if(!InitWiFi(0)) // try quick connection
			InitWiFi(1);   // try longer to wait for connection

	if(!WIFI_LINK()) return 0; // no wifi connection cannot send to host

	WriteWIFIDATA(data,"");

	while(!SerialAvailable(WIFI_UART) && timeleft) // wait for response or timeout
		delay(1*millisecond);
  ...
  uint8 k=0;
	...
	while(1)	// Keep in this loop until connected to an access point
	{
  	CLS();
		...
		WaitEvent(); // go to low power idle while event comes in
